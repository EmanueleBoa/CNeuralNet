...
void init_layer(layer *l, char *type, int n, int nprev) {
  ...
  if(nprev > 0) {
    ...
    l->weights = (double *) malloc(n * nprev * sizeof(double));
    ...
    for(i=0; i<n*nprev; i++) {
        ...
        l->weights[i] = ran_gauss(0.0, sqrt(2.0/(l->n+l->nprev)));
        //l->weights[i][j] = 0.01;
      }
  }
}
...
void init_gradients(layer *l) {
  ...
  l->delta_weights = (double *) malloc(l->m * sizeof(double));
  l->grad_biases = (double *) malloc(l->n * sizeof(double));
  l->grad_weights = (double *) malloc(l->m * sizeof(double));
  ...
  for(i=0; i<l->n; i++) {
    ...
    l->grad_biases[i] = 0.0;
    for(j=0; j<l->nprev; j++) {
      l->delta_weights[k] = 0.0;
      l->grad_weights[k] = 0.0;
      k++;
    }
  }
}
...
void linear_activation(layer *l, double *units_prev) {
  ...
  for(k=0; k<l->n; k++){
    l->units_lin[k] = l->biases[k];
  }
  cblas_dgemv(CblasRowMajor, CblasNoTrans, l->n, l->nprev, 1.0, l->weights, l->nprev, units_prev, 1, 1.0, l->units_lin, 1);
}
...
void delta(layer *l, layer *lnext) {
  ...
  // compute deltas
  cblas_dgemv(CblasRowMajor, CblasTrans, lnext->n, l->n, 1.0, lnext->weights, l->n, lnext->deltas, 1, 0.0, l->deltas, 1);
  for(i=0; i<l->n; i++) {
    l->deltas[i] *= fprime[i];
  }
...
void update_gradients(layer *l, layer *lprev) {
  ...
  // weights' gradients
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, l->n, l->nprev, 1, 1.0, l->deltas, 1, lprev->units_act, l->nprev, 1.0, l->grad_weights, l->nprev);
}
...
void gradient_descent(layer *l, double rate, double momentum, double weight_decay, int batchsize) {
  ...
  for(i=0; i<l->n; i++) {
    ...
    for(j=0; j<l->nprev; j++) {
      l->delta_weights[k] = momentum*l->delta_weights[k]
      -rate*(l->grad_weights[k]/batchsize+weight_decay*l->weights[k]);
      l->weights[k] += l->delta_weights[k];
      k++;
    }
  }
  ...
  for(n=1; n<nl; n++) {
    ...
    for(i=0; i<net->layers[n].n; i++) {
      for(j=0; j<net->layers[n].nprev; j++) {
        fscanf(in, "%lg", &temp);
        net->layers[n].weights[k] = temp;
        k++;
        //printf("%lg ", net->layers[n].weights[k]);
      }
      //printf("\n");
    }
  }
...
void save_net(NN *net, char *file_name) {
  ...
  for(n=1; n<net->nl; n++) {
    ...
    for(i=0; i<net->layers[n].n; i++) {
      for(j=0; j<net->layers[n].nprev; j++) {
        fprintf(out, "%lg ", net->layers[n].weights[k]);
        k++;
      }
      fprintf(out, "\n");
    }
